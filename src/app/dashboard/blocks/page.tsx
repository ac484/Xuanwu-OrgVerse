 ...
 * 不再涉及「佈署」動作，僅提供技術規範的查看，體現「獨立業務能力模組」的隔離性。
 */
          ...
          <Card key={block.id} className="border-border/60 hover:border-primary/40 transition-all group bg-card/50 backdrop-blur-sm">
            ...
            <CardContent>
              <div className="flex items-center gap-2">
                <Badge variant="outline" className="text-[9px] uppercase tracking-tighter">類型: {block.type}</Badge>
                <Badge variant="outline" className="text-[9px] uppercase tracking-tighter">維度範圍: {activeOrg.name}</Badge>
              </div>
            </CardContent>
            <CardFooter className="pt-0 border-t border-border/20 py-4 flex justify-between items-center">
              <span className="text-[10px] font-bold text-muted-foreground uppercase">SIG ID: {block.id}</span>
              <Button variant="ghost" size="sm" className="h-8 gap-2 font-bold uppercase text-[9px] tracking-widest text-primary">
                <Info className="w-3.5 h-3.5" /> 查看架構定義 (Schema)
              </Button>
            </CardFooter>
          </Card>
        ))}
