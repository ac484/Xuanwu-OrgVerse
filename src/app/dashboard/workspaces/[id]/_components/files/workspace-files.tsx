...
export function WorkspaceFiles() {
  ...
  const getFileIcon = (fileName: string) => {
    ...
    switch (ext) {
      ...
      case 'zip': case '7z': return <FileArchive className="w-5 h-5" />;
      case 'ts': case 'tsx': return <FileCode className="w-5 h-5" />;
            ...
            return (
              <div key={file.id} className="grid grid-cols-12 gap-4 px-6 py-4 items-center group hover:bg-primary/[0.02] transition-colors">
                ...
                <div className="col-span-2 flex flex-col">
                  ...
                  <span className="text-[9px] text-muted-foreground flex items-center gap-1 font-medium"><Clock className="w-2.5 h-2.5" /> 剛剛</span>
                </div>
                <div className="col-span-1 flex justify-end">
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end" className="rounded-xl w-48">
                      <DropdownMenuItem onClick={() => setHistoryFile(file)} className="gap-2 text-[10px] font-bold uppercase py-2.5"><History className="w-3.5 h-3.5 text-primary" /> 版本歷程治理</DropdownMenuItem>
                      <DropdownMenuItem className="gap-2 text-[10px] font-bold uppercase py-2.5 text-destructive"><Trash2 className="w-3.5 h-3.5" /> 註銷檔案</DropdownMenuItem>
                    </DropdownMenuContent>
                  </DropdownMenu>
                </div>
              </div>
              ...
              {historyFile?.versions?.slice().reverse().map((v: any) => (
                <div key={v.versionId} className="relative">
                  <div className={cn(
                    ...
                    historyFile.currentVersionId === v.versionId ? "bg-primary ring-primary/20 scale-125" : "bg-muted ring-muted/20"
                  )} />
                  <div className={cn("p-5 rounded-2xl border", historyFile.currentVersionId === v.versionId ? "bg-primary/5 border-primary/30" : "bg-muted/30 border-border/60")}>
                    <div className="flex items-center justify-between mb-2">
                      <p className="text-xs font-black">{v.versionName}</p>
                      {historyFile.currentVersionId === v.versionId && <Badge className="text-[8px] bg-primary uppercase font-black">Active</Badge>}
                    </div>
                    ...
                    {historyFile.currentVersionId !== v.versionId && (
                      <Button variant="outline" size="sm" className="w-full mt-4 h-8 text-[9px] font-black uppercase bg-background" onClick={() => handleRestore(historyFile, v.versionId)}>
